...
{
   qstr->buffer = Z_Realloc(qstr->buffer, (int)size, PU_STATIC, NULL);
   qstr->size   = size;
   qstr->index  = 0;
...
{
   unsigned int cursize = (unsigned int)qstr->size;
   unsigned int newsize = (unsigned int)qstr->index + (unsigned int)strlen(str) + 1;
   ...
   qstr->index = (unsigned int)strlen(qstr->buffer);
...
{
   return w3sstrcasecmp(qstr->buffer, str);
}
...
{
   return w3sstrncasecmp(qstr->buffer, str, maxcount);
}
