...
static void RB_SetupAutomapView(rbView_t *view)
{
	...
	glViewport(0, delta, screen->w, screen->h);
#else
	int w;
	int h;
...
void RB_DrawAutomapLine(fixed_t x1, fixed_t y1, fixed_t x2, fixed_t y2, int color)
{
    ...
    //
    glBegin(GL_LINES);
    glColor4ub(rgb[0], rgb[1], rgb[2], 0xff);
...
void RB_BeginAutomapDraw(void)
{
    ...
    glLoadMatrixf(rbAutomapView.projection);

    // load modelview
    glMatrixMode(GL_MODELVIEW);
    glLoadMatrixf(rbAutomapView.modelview);
