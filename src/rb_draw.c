...
void RB_DrawDamageMarkers(player_t *player)
{
    ...
    for(dmgmarker = dmgmarkers.next; dmgmarker != &dmgmarkers; dmgmarker = dmgmarker->next)
    {
        ...
        glPushMatrix();
        glTranslatef(160, viewheight != SCREENHEIGHT ? 88 : 120, 0);
...
void RB_RenderBloom(void)
{
    ...
    glPushAttrib(GL_VIEWPORT_BIT);
    glViewport(0, 0, bloomFBO.fboWidth, bloomFBO.fboHeight);
...
void RB_DrawScene(void)
{
    ...
    glLoadMatrixf(rbPlayerView.projection);
    ...
    // load modelview
    glMatrixMode(GL_MODELVIEW);
    glLoadMatrixf(rbPlayerView.modelview);
